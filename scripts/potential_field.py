...
class BehaviorPotentialField:
    ...
    def show_bpf(self):
        ...
        for h in range(len(self.humans)):
            ...
            # print "bpf_array's shape : "+str(bpf_array.shape)
            # print "(F_max, F_min) = ("+str(F_max)+", "+str(F_min)+")"
        ...
        ## The following code is just show the img in valious ways
        ## Show the bpf_array(plt)
        # plt.plot(), plt.imshow(bpf_array, cmap='gray', vmin=0, vmax=255)
        # plt.show()
        ...
        ## apply color map on the image (maybe you don't need)
        # apply_color_map_image = cv2.applyColorMap(bpf_array, cv2.COLORMAP_AUTUMN)
        # cv2.imshow('appColorMap',apply_color_map_image)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
